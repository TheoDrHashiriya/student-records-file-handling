...
private:
    ...
    std::string getStringInput(const std::string& prompt, bool allowSpaces = false) {
        std::string input;
        std::cout << prompt;
    ...
    void printHeader() {
        ...
        std::cout << std::setw(colWidth) << "Student ID" 
                 << std::setw(colWidth) << "Name" 
                 << std::setw(colWidth) << "Course" 
                 << std::setw(colWidth) << "Year Level" 
                 << std::setw(colWidth) << "Final Grade" << "\n";
        std::cout << std::string(colWidth * 5, '-') << "\n";
    }
    
    // Print a single student record
    void printStudent(const Student& student) {
        std::cout << std::setw(colWidth) << student.studentID 
                 << std::setw(colWidth) << student.name 
                 << std::setw(colWidth) << student.course 
                 << std::setw(colWidth) << student.yearLevel 
                 << std::setw(colWidth) << std::fixed << std::setprecision(2) << student.finalGrade << "\n";
    }
    
public:
    StudentManagementSystem(const std::string& file) : fileName(file) {
        if (!loadRecords()) {
            std::cout << "Warning: Could not load file '" << fileName << "'. A new file will be created when records are added.\n";
        } else {
            std::cout << "Successfully loaded " << students.size() << " records from " << fileName << ".\n";
        }
    }
    ...
    void addStudent() {
        ...
        do {
            ...
            if (studentExists(student.studentID)) {
                std::cout << "Error: A student with ID " << student.studentID << " already exists.\n";
            } else {
                validID = true;
            }
        } while (!validID);
        ...
        student.yearLevel = getValidInput<int>("Enter Student Year Level (1-6): ", 1, 6);
        student.finalGrade = getValidInput<float>("Enter Student Final Grade (0-100): ", 0.0f, 100.0f);
        ...
        if (it != students.end()) {
            ...
            do {
                ...
                if (input.empty()) {
                    ...
                    std::cout << "Error: A student with ID " << input << " already exists.\n";
                } else {
                    it->studentID = input;
                    validID = true;
                }
            ...
            std::cout << "Enter new Year Level (1-6, or press Enter to keep current): ";
            std::getline(std::cin, input);
            if (!input.empty()) {
                try {
            ...
            std::cout << "Enter new Final Grade (0-100, or press Enter to keep current): ";
            std::getline(std::cin, input);
            if (!input.empty()) {
                try {
